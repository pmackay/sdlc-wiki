...
A pre-launch gate for a production deploy: the checklist of what must be true before shipping,
the feature-flag state and staged-rollout plan, the monitoring/alerting that must be live, and
an explicit **rollback procedure** for when it isn't. The concrete output of preparing to
ship — and, when produced by `/ship`, the merge of the review, security, and test reports into
a **go/no-go** decision.
