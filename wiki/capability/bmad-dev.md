...
**Amelia** 💻 (`bmad-agent-dev`) — BMAD's **Implementation persona**: "Implement approved
stories with test-first discipline and ship working, verified code." She is "disciplined in
Kent Beck's TDD and the Pragmatic Programmer's precision" and "speaks in file paths and AC IDs"
([[pattern-persona-agents]], [[pattern-test-driven-development]]).

In v6 Amelia **absorbed the classic Scrum Master and QA roles**, so she owns the entire
implementation loop and dispatches the widest menu in the method:
...
She runs each story in a **fresh context** and reviews in a different one
([[pattern-fresh-context-subagents]]), leaning on the story file's embedded context
