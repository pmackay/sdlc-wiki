...
Its distinctive stance is a **spine of invariants only** — "a consistency contract that fixes only the invariants … the design paradigm, the boundary and dependency rules, how state is mutated, who owns shared data." Everything structural (stack, tree, full data shape) is treated as "seed: true at cold-start, owned by the code once it exists." That is BMAD's [[pattern-scale-adaptive-planning|just-in-time]] answer to over-specified up-front design.
