...
`/learn` — **Memory**. Manage what gstack learned across sessions: review, search, prune, and export project-specific **patterns, pitfalls, and preferences**. "Learnings compound across sessions so gstack gets smarter on your codebase over time." Shares storage with the browser **domain skills** ([[gstack-skillify]]) and, when configured, the gbrain persistent memory ([[gstack-setup-gbrain]]).

gstack's **agent-grounding** flavor of [[stage-learn]] — the direct counterpart to Compound Engineering's [[ce-compound]] / [[ce-compound-refresh]] (a maintained, machine-consumable learnings corpus that future runs auto-read → [[artifact-solution-doc]], [[pattern-knowledge-compounding]]). Its team-process sibling is [[gstack-retro]]. Together they make gstack the **third framework** to evidence the learn stage.
