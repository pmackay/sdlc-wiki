...
**`bmad-document-project`** — "Document brownfield projects for AI context." Dispatched by
[[bmad-tech-writer]] (and [[bmad-analyst]], menu `DP`), it scans an existing codebase — full
scan or targeted deep-dive — and produces an index, overview, source-tree map, and per-area
deep-dives so agents can work an unfamiliar codebase with full context ([[pattern-context-engineering]]).
